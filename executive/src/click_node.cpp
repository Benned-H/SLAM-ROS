...
{	
	...
	ros::Subscriber clicked_point_sub = node_handle.subscribe("clicked_point",
		1, &ClickHandler::handleClick, &click);
	
	// Set up any publishers inside the class instance.
	click.waypoint_pub = node_handle.advertise<geometry_msgs::Point>
		("executive/waypoint", 1, true);
