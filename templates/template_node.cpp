...
int main(int argc, char* argv[])
{
	...
	obj.datatype_pub = node_handle.advertise<package_name::DataType>
		("topic_name", 1, true);
	...
	ros::Publisher datatype_pub = node_handle.advertise<package_name::DataType>
		("topic_name2", 1, true); // Same meaning as above.
		
	sleep(1); // Wait 1 second; gives time for ROS connections to be made.
		
	// You could then instantiate, fill in, and publish a message without
	// needing the package's class at all:
	...
	while (ros::ok()) // Keeps looping as long as the node is running.
	{
		...
		obj.step(1.0/frequency); // Advance the class' simulation/belief.
